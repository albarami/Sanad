...
async def test_agents():
    ...
    try:
        ...
        agent_input = AgentInput(
            question=question, draft_answer=draft_answer, passages=passages
        )
        ...
        for agent in agents:
            try:
                ...
                print(
                    f"✅ {agent.name}: score={result.score:.3f}, confidence={result.confidence:.3f}"
                )
...
async def test_full_system():
    ...
    try:
        ...
        # Verification
        coordinator_input = CoordinatorInput(
            question=question, draft_answer=draft.answer, passages=passages
        )
        ...
        # Success criteria
        if (
            result.sanad_score > 0.0 and result.processing_time_ms < 10000
        ):  # 10 second timeout
