...
class IntegrityAgent(BaseAgent):
    ...
    async def evaluate(self, input_data: AgentInput) -> AgentScore:
        ...
        try:
            ...
            explanation = self._generate_explanation(
                authority_score, contradiction_score, consistency_score, input_data.passages
            )
            
            logger.debug(f"IntegrityAgent: authority={authority_score:.3f}, "
    ...
    def _assess_source_authority(self, passages: List[Any]) -> float:
        ...
        logger.debug(f"Source authority assessment: {average_authority:.3f} from {len(passages)} passages")
    ...
    async def _check_contradictions(self, input_data: AgentInput) -> float:
        ...
        try:
            ...
            if "no contradictions" in contradiction_assessment or "consistent" in contradiction_assessment:
                return 1.0
            elif "minor inconsistency" in contradiction_assessment or "slight variation" in contradiction_assessment:
                return 0.8
            elif "contradiction" in contradiction_assessment or "inconsistent" in contradiction_assessment:
                return 0.3
            else:
    ...
    def _build_contradiction_prompt(self, question: str, answer: str, passages: List[Any]) -> str:
        """
...
Assessment:"""
    ...
    def _generate_explanation(self, authority_score: float, contradiction_score: float, 
                            consistency_score: float, passages: List[Any]) -> str:
        """
        ...
        if authority_score >= 0.8:
            explanations.append("High-authority sources (government/official documents)")
