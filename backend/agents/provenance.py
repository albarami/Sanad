...
class ProvenanceAgent(BaseAgent):
    ...
    async def evaluate(self, input_data: AgentInput) -> AgentScore:
        ...
        try:
            ...
            coverage_score = self._assess_citation_coverage(input_data.draft_answer, input_data.passages)
            
            # Step 3: Evaluate source traceability
            traceability_score = self._evaluate_traceability(citations, input_data.passages)
            ...
            explanation = self._generate_explanation(
                citations, coverage_score, traceability_score, 
                diversity_score, quality_score, input_data.passages
            )
            
            logger.debug(f"ProvenanceAgent: coverage={coverage_score:.3f}, "
    ...
    def _assess_citation_coverage(self, answer: str, passages: List[Any]) -> float:
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 10:  # Ignore very short sentences
                ...
                if any(indicator in sentence.lower() for indicator in [
                    'is', 'are', 'must', 'shall', 'according to', 'states',
                    'requires', 'allows', 'prohibits', 'specifies', '%', 'days',
                    'hours', 'article', 'section', 'law', 'regulation'
                ]):
        ...
        logger.debug(f"Citation coverage: {citation_count} citations for {len(factual_sentences)} factual sentences")
        return coverage_ratio
    
    def _evaluate_traceability(self, citations: List[Dict[str, Any]], passages: List[Any]) -> float:
        """
        ...
        for citation in citations:
            ...
            elif any(legal_term in reference for legal_term in ['article', 'section', 'law']) and \
                 any('law' in source_id for source_id in source_ids):
                traceable_count += 1
        
        traceability_ratio = traceable_count / len(citations) if citations else 0.6
        
        logger.debug(f"Traceability: {traceable_count}/{len(citations)} citations traceable to sources")
    ...
    def _assess_source_diversity(self, passages: List[Any]) -> float:
        ...
        for passage in passages:
            ...
            if any(indicator in doc_id for indicator in self.source_quality_indicators["official"]):
                source_types.add("official")
            elif any(indicator in doc_id for indicator in self.source_quality_indicators["academic"]):
                source_types.add("academic")
            elif any(indicator in doc_id for indicator in self.source_quality_indicators["regulatory"]):
    ...
    def _validate_source_quality(self, passages: List[Any]) -> float:
        ...
        logger.debug(f"Source quality: average {average_quality:.3f} from {len(passages)} sources")
    ...
    def _assess_citation_strength(self, citation_text: str) -> float:
        ...
        if re.search(r'article \d+', citation_lower) or re.search(r'section \d+', citation_lower):
    ...
    def _generate_explanation(self, citations: List[Dict[str, Any]], coverage_score: float,
                            traceability_score: float, diversity_score: float,
                            quality_score: float, passages: List[Any]) -> str:
        """
