...
try:
    from .base import BaseAgent, AgentInput, AgentScore
...
class DomainLabourAgent(BaseAgent):
    ...
    def __init__(self):
        ...
        self.labour_law_patterns = {
            # Key legal concepts
            "employment_contract": [
                r"employment contract", r"work contract", r"عقد العمل", r"عقد التوظيف"
            ],
            "probation_period": [
                r"probation(?:ary)?\s+period", r"trial period", r"فترة التجربة", r"فترة الاختبار"
            ],
            "termination": [
                r"terminat(?:e|ion)", r"dismiss(?:al)?", r"end(?:ing)?\s+(?:of\s+)?employment",
                r"إنهاء", r"فصل", r"إقالة"
            ],
            "notice_period": [
                r"notice\s+period", r"advance\s+notice", r"مدة الإشعار", r"فترة الإخطار"
            ],
            "working_hours": [
                r"working\s+hours?", r"work(?:ing)?\s+time", r"ساعات العمل", r"وقت العمل"
            ],
            "overtime": [
                r"overtime", r"extra\s+hours?", r"العمل الإضافي", r"ساعات إضافية"
            ],
            "annual_leave": [
                r"annual\s+leave", r"vacation", r"holiday", r"الإجازة السنوية", r"عطلة"
            ],
            "sick_leave": [
                r"sick\s+leave", r"medical\s+leave", r"إجازة مرضية"
            ],
            "wages": [
                r"wages?", r"salary", r"compensation", r"pay(?:ment)?", r"راتب", r"أجر", r"مرتب"
            ],
            "rights": [
                r"rights?", r"entitlement", r"حقوق", r"استحقاق"
            ],
            "obligations": [
                r"obligation", r"duties", r"responsibilities", r"واجبات", r"التزامات"
            ]
        }
        ...
        self.qatar_legal_refs = {
            "labour_law": [
                r"qatar(?:i)?\s+labo?ur\s+law", r"law\s+no\.?\s*14", r"قانون العمل القطري",
                r"القانون رقم 14"
            ],
            "ministry_labour": [
                r"ministry\s+of\s+(?:administrative\s+development\s+)?labo?ur",
                r"وزارة العمل", r"وزارة التنمية الإدارية والعمل"
            ],
            "articles": [
                r"article\s+(\d+)", r"المادة\s+(\d+)"
            ]
        }
    ...
    async def evaluate(self, input_data: AgentInput) -> AgentScore:
        ...
        try:
            ...
            relevance_score = self._assess_domain_relevance(input_data.question, input_data.draft_answer)
            ...
            qatar_context_score = self._check_qatar_context(input_data.draft_answer, input_data.passages)
            ...
            explanation = self._generate_explanation(
                relevance_score, pattern_score, qatar_context_score, 
                values_score, compliance_score
            )
            
            logger.debug(f"DomainLabourAgent: relevance={relevance_score:.3f}, "
    ...
    def _assess_domain_relevance(self, question: str, answer: str) -> float:
        ...
        logger.debug(f"Domain relevance: {total_matches} matches across {categories_matched} categories")
    ...
    def _validate_legal_patterns(self, answer: str) -> float:
        ...
        logger.debug(f"Legal patterns: {proper_terms}/{total_legal_concepts} proper usage, {qatar_refs} Qatar refs")
    ...
    def _check_qatar_context(self, answer: str, passages: List[Any]) -> float:
        ...
        qatar_indicators = [
            r"qatar(?:i)?", r"qr", r"doha", r"gcc",
            r"قطر", r"قطري", r"الدوحة"
        ]
        
        answer_lower = answer.lower()
        qatar_mentions = sum(1 for pattern in qatar_indicators 
                           if re.search(pattern, answer_lower, re.IGNORECASE))
        ...
        for passage in passages:
            ...
            if any(indicator in doc_id for indicator in ['qatar', 'labour_law', 'qnds']):
                qatar_sources += 1
        
        # Calculate context score
        answer_context = min(qatar_mentions * 0.3, 0.6)  # Up to 0.6 from answer mentions
        
        if total_sources > 0:
            source_context = (qatar_sources / total_sources) * 0.4  # Up to 0.4 from sources
        else:
            source_context = 0.2  # Default when no sources
        
        context_score = answer_context + source_context
        
        logger.debug(f"Qatar context: {qatar_mentions} answer mentions, {qatar_sources}/{total_sources} Qatar sources")
    ...
    async def _validate_legal_values(self, input_data: AgentInput) -> float:
        ...
        try:
            ...
            if "accurate" in validation_result and "correct" in validation_result:
                return 0.9
            elif "mostly accurate" in validation_result or "largely correct" in validation_result:
                return 0.8
    ...
    def _assess_compliance_language(self, answer: str) -> float:
        ...
        positive_indicators = [
            r"according to", r"as per", r"in compliance with", r"pursuant to",
            r"shall", r"must", r"required", r"mandatory", r"stipulated",
            r"governed by", r"subject to", r"in accordance with"
        ]
        
        # Negative indicators (vague or non-compliant language)
        negative_indicators = [
            r"might", r"maybe", r"possibly", r"generally", r"usually",
            r"often", r"sometimes", r"could be", r"may be"
        ]
        
        positive_count = sum(1 for pattern in positive_indicators 
                           if re.search(pattern, answer_lower))
        negative_count = sum(1 for pattern in negative_indicators 
                           if re.search(pattern, answer_lower))
        ...
        logger.debug(f"Compliance language: {positive_count} positive, {negative_count} negative indicators")
    ...
    def _is_proper_legal_usage(self, pattern: str, text: str) -> bool:
        ...
        for match in matches:
            ...
            legal_context_indicators = [
                "according to", "as per", "law", "article", "section",
                "regulation", "provision", "clause", "statute", "legal",
                "employment", "employer", "employee", "contract"
            ]
    ...
    def _build_values_validation_prompt(self, question: str, answer: str, 
                                      values: List[Dict[str, Any]], passages: List[Any]) -> str:
        """
...
Validation result:"""
    ...
    def _generate_explanation(self, relevance_score: float, pattern_score: float,
                            qatar_context_score: float, values_score: float,
