...
class PrecisionAgent(BaseAgent):
    ...
    async def evaluate(self, input_data: AgentInput) -> AgentScore:
        ...
        try:
            ...
            logger.debug(f"PrecisionAgent: factual={factual_score:.3f}, "
    ...
    async def _evaluate_claim_support(self, input_data: AgentInput) -> float:
        ...
        try:
            ...
            logger.debug(f"Claim support: {supported_claims}/{total_claims} claims supported")
    ...
    def _assess_specificity(self, answer: str) -> float:
        ...
        specificity_indicators = {
            ...
            "dates": len(re.findall(r'\b\d{4}\b|\b\d{1,2}/\d{1,2}/\d{2,4}\b', answer)) * 0.15,
            "percentages": len(re.findall(r'\b\d+(?:\.\d+)?%\b', answer)) * 0.2,
            "proper_nouns": len(re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', answer)) * 0.05,
            
            # Specific language vs vague language
            "specific_terms": sum(0.1 for term in ['exactly', 'specifically', 'precisely', 'according to'] 
                                if term in answer.lower()),
            "vague_penalty": sum(-0.1 for term in ['generally', 'usually', 'often', 'sometimes', 'might', 'may'] 
                               if term in answer.lower())
        }
        ...
        length_bonus = min(len(answer.split()) / 100, 0.2)  # Up to 0.2 bonus for longer answers
        ...
        for sentence in sentences:
            ...
            if len(sentence) > 10:  # Ignore very short sentences
                ...
                if any(indicator in sentence.lower() for indicator in [
                    'is', 'are', 'must', 'shall', 'should', 'according to',
                    'requires', 'allows', 'prohibits', 'states', 'specifies'
                ]):
...
Assessment:"""
    ...
    def _build_factual_verification_prompt(self, question: str, answer: str, passages: List[Any]) -> str:
        """
        Build prompt for factual verification.
...
Accuracy assessment:"""
    ...
    def _generate_explanation(self, factual_score: float, claim_support_score: float,
                            specificity_score: float, relevance_score: float) -> str:
        """
