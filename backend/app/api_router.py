...
try:
    ...
    from ..db.repository import UserRepository, QueryRepository, FeedbackRepository, AuditRepository, ComplianceRepository
    from ..db.models import User, UserCreate, UserResponse, QueryLogResponse, FeedbackCreate, AuditLogCreate
except ImportError:
...
class VerifyRequest(BaseModel):
    ...
    question: str = Field(..., min_length=1, max_length=4000, description="User question to verify")
...
class BaselineRequest(BaseModel):
    ...
    question: str = Field(..., min_length=1, max_length=4000, description="Question for baseline LLM")
...
class DemoRequest(BaseModel):
    ...
    question: str = Field(..., min_length=1, max_length=4000, description="Question for baseline vs Sanad comparison")
    include_performance_metrics: bool = Field(default=True, description="Include detailed performance analysis")
...
async def get_baseline_answer(
    ...
    try:
        ...
        if baseline_llm is None:
            raise HTTPException(status_code=503, detail="Baseline LLM service not available")
...
@api_router.post("/verify", response_model=EnhancedVerificationResponse, tags=["Verification"])
async def verify_answer(
    ...
    try:
        ...
        if not should_trigger:
            processing_time = int((time.time() - start_time) * 1000)
            logger.info(f"No verification needed, returning baseline answer in {processing_time}ms")
        ...
        if retriever:
            try:
                ...
                retrieval_results = retriever.search(request.question, k=5, category=category)
                ...
                logger.info(f"Retrieved {len(passages)} passages from category: {category}")
                
            except Exception as e:
                logger.warning(f"Retrieval failed: {str(e)}, proceeding without passages")
                passages = []
        else:
        ...
        if coordinator and len(passages) > 0:
            ...
            logger.info(f"Verification completed with score: {verification_result.sanad_score:.3f}")
            ...
            logger.warning("Coordinator or passages not available, returning enhanced baseline")
            processing_time = int((time.time() - start_time) * 1000)
...
@api_router.get("/queries/history", response_model=List[QueryLogResponse], tags=["Enterprise"])
async def get_query_history(
    ...
    try:
        ...
        query_repo = QueryRepository(db)
        queries = query_repo.get_user_queries(current_user.id, limit=limit, offset=offset)
...
@api_router.post("/demo/comparison", response_model=EnterpriseDemoResponse, tags=["Enterprise Demo"])
async def enterprise_demo_comparison(
    ...
    logger.info(f"Enterprise demo requested: {request.question[:50]}... by {current_user.user_id if current_user else 'anonymous'}")
    ...
    try:
        ...
        if baseline_llm is None:
            raise HTTPException(status_code=503, detail="Demo service not available - LLM not initialized")
        ...
        # Check trigger
        should_trigger = trigger_detector.use_sanad(request.question) if trigger_detector else True
        trigger_reason = trigger_detector.get_trigger_reason(request.question) if trigger_detector else "Manual demo"
        ...
        if not should_trigger:
            ...
            if coordinator and len(passages) > 0:
                ...
                avg_score = sum(p.score for p in passages) / len(passages) if passages else 0.3
        ...
        time_overhead = ((sanad_result.processing_time_ms - baseline_result.processing_time_ms) / baseline_result.processing_time_ms * 100) if baseline_result.processing_time_ms > 0 else 0
        ...
        capability_comparison = {
            "verification": {
                "baseline": "None",
                "sanad": f"Score: {sanad_result.verification_score:.2f}" if sanad_result.verification_score else "Limited"
            },
            "sources": {
                "baseline": "0 sources",
            ...
            "compliance": {
                "baseline": "Not assured",
                "sanad": "Document-verified"
            }
        }
        ...
        business_value_assessment = {
            "document_coverage": f"{source_count} relevant documents found" if source_count > 0 else "No relevant documents in corpus",
            "verification_status": "Verified against official sources" if source_count > 0 else "Limited verification available",
            "compliance_readiness": "Enterprise-ready" if sanad_result.verification_score and sanad_result.verification_score > 0.7 else "Requires review",
            "risk_reduction": "High" if source_count > 0 and sanad_result.verification_score and sanad_result.verification_score > 0.8 else "Medium" if source_count > 0 else "Limited",
            "recommended_action": "Deploy to production" if source_count > 0 and sanad_result.verification_score and sanad_result.verification_score > 0.7 else "Expand document corpus" if source_count == 0 else "Review verification settings"
        }
        ...
        logger.info(f"Enterprise demo completed in {total_time}ms: {source_count} sources, score={sanad_result.verification_score}")
