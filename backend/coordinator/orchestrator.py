...
class SanadCoordinator:
    ...
    async def verify(
        ...
        try:
            ...
            if sanad_score < enhancement_threshold:
                logger.info(
                    f"Score {sanad_score:.3f} below threshold {enhancement_threshold}, enhancing answer"
                )
                final_answer = await self._enhance_answer(
                    input_data, agent_scores, sanad_score
                )
                enhanced = True
            else:
                logger.info(
                    f"Score {sanad_score:.3f} above threshold, using draft answer"
                )
    ...
    async def _execute_agents(
        self, input_data: CoordinatorInput
    ) -> Dict[str, AgentScore]:
        """
    ...
    async def _safe_agent_execution(
        self, agent_name: str, agent: BaseAgent, input_data: AgentInput
    ) -> Optional[AgentScore]:
        """
        ...
        try:
            ...
            score = await asyncio.wait_for(
                agent.evaluate(input_data), timeout=0.85  # 850ms
            )
            return score
    ...
    def _compute_sanad_score(self, agent_scores: Dict[str, AgentScore]) -> float:
        ...
        for agent_name, agent_score in agent_scores.items():
            if agent_name in weight_mapping:
                ...
                total_weight += weight
                logger.debug(
                    f"Agent {agent_name}: score={agent_score.score:.3f}, weight={weight}"
                )
        ...
        logger.info(
            f"Computed Sanad score: {final_score:.3f} (from {len(agent_scores)} agents)"
        )
    ...
    async def _log_verification(
        ...
        try:
            ...
            # Extract source IDs
            sources_used = (
                [passage.doc_id for passage in input_data.passages]
                if input_data.passages
                else []
            )
