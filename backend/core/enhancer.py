...
try:
    ...
    ENHANCEMENT_ATTEMPTS = get_or_create_metric(
        Counter,
        "sanad_enhancement_attempts_total",
        "Total enhancement attempts",
        ["status"],
    )
    ENHANCEMENT_DURATION = get_or_create_metric(
        Histogram, "sanad_enhancement_duration_seconds", "Enhancement processing time"
    )
    METRICS_AVAILABLE = True
except ImportError:
    METRICS_AVAILABLE = False
    logger.warning(
        "Prometheus metrics not available - install prometheus_client for production monitoring"
    )
...
class ResponseEnhancer:
    ...
    def __init__(
        self, config: Optional[Config] = None, llm: Optional[BaselineLLM] = None
    ):
        ...
        logger.info(
            f"Initialized ResponseEnhancer with threshold: {self.enhancement_threshold}"
        )
    ...
    def reload_config(self, config: Optional[Config] = None) -> None:
        ...
        logger.info(
            f"Reloaded ResponseEnhancer config with {len(self._compiled_keywords)} keywords"
        )
    ...
    async def enhance(self, request: EnhancementRequest) -> EnhancementResponse:
        ...
        try:
            ...
            if not self._should_enhance(request):
                logger.info(
                    f"Enhancement not needed - Sanad score {request.sanad_score:.3f} >= threshold {self.enhancement_threshold}"
                )
                if METRICS_AVAILABLE:
            ...
            # Perform enhancement
            logger.info(
                f"Enhancing answer - Sanad score {request.sanad_score:.3f} < threshold {self.enhancement_threshold}"
            )
            enhanced_answer, enhancement_success = await self._perform_enhancement(
                request
            )
            ...
            return EnhancementResponse(
                ...
                enhancement_reason=(
                    f"Sanad score {request.sanad_score:.3f} below threshold"
                    if enhancement_success
                    else "Enhancement failed"
                ),
                confidence=(
                    min(request.sanad_score + 0.1, 1.0)
                    if enhancement_success
                    else request.sanad_score
                ),
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt == max_retries - 1:
                    ...
                    logger.warning(
                        f"LLM timeout, retrying in {delay:.2f}s (attempt {attempt + 1}/{max_retries})"
                    )
                ...
                if attempt == max_retries - 1:
                    ...
                    delay = base_delay * (2**attempt) * (1 + random.uniform(-0.2, 0.2))
                    logger.warning(
                        f"LLM error: {str(e)}, retrying in {delay:.2f}s (attempt {attempt + 1}/{max_retries})"
                    )
                    await asyncio.sleep(delay)
    ...
    def _build_enhancement_prompt(self, request: EnhancementRequest) -> str:
        ...
        for score in request.agent_scores.values():
            if score.explanation:
                agent_feedback.append(
                    f"- {score.agent_name.title()}: {score.explanation}"
                )

        agent_feedback_text = (
            "\n".join(agent_feedback)
            if agent_feedback
            else "No specific agent feedback available."
        )

        # Determine enhancement focus based on lowest scoring agents
        lowest_agents = sorted(request.agent_scores.values(), key=lambda x: x.score)[:2]
        ...
        domain_instructions = ""
        if (
            self.config.domain.enhancement_instructions
            and self.config.domain.enhancement_instructions.strip()
            and self._is_domain_relevant(request.question)
        ):
            domain_instructions = (
...
IMPROVED ANSWER:"""
    ...
    def _is_domain_relevant(self, question: str) -> bool:
        ...
        if self._compiled_keywords and len(self._compiled_keywords) == len(
            self.config.domain.keywords
        ):
