...
class SanadCLI:
    ...
    def _process_query(self, query: str):
        ...
        trigger_reason = self.detector.get_trigger_reason(query) if should_trigger else "No verification needed"
        ...
        if should_trigger:
            ...
            for i, passage in enumerate(passages, 1):
                text_preview = passage['text'][:80] + "..." if len(passage['text']) > 80 else passage['text']
            ...
            avg_retrieval_score = sum(p['score'] for p in passages[:3]) / 3 if passages else 0.0
            mock_sanad_score = min(0.95, avg_retrieval_score + 0.3)  # Boost score for demo
        ...
        if should_trigger:
            if elapsed_time <= 1000:
                self.console.print("[bold green]✓ Performance target met (≤1000ms)![/bold green]")
            else:
                self.console.print(f"[bold yellow]⚠ Performance target missed (target: ≤1000ms)[/bold yellow]")
