...
class BenchmarkRunner:
    ...
    def evaluate_answer(self, expected: str, actual: str, category: str) -> bool:
        ...
        if category == "labor_market":
            ...
            # Key labor terms
            labor_terms = ["wage", "hours", "leave", "notice", "overtime", "maternity", "safety", "permit"]
            ...
            finance_terms = ["riba", "sharia", "murabaha", "musharaka", "takaful", "halal", "haram"]
        ...
        if not key_terms:
            ...
            return any(word in actual_lower for word in expected_lower.split() if len(word) > 3)
    ...
    def print_summary(self, analysis: Dict) -> None:
        ...
        print(f"\n🎯 Overall Performance:")
        print(f"Accuracy: {analysis['overall_accuracy']:.1%} ({analysis['correct_answers']}/{analysis['total_questions']})")
        print(f"Target:   {analysis['target_accuracy']:.1%}")
        print(f"Status:   {'✅ PASS' if analysis['meets_accuracy_target'] else '❌ FAIL'}")
        ...
        print(f"Status:   {'✅ PASS' if analysis['meets_latency_target'] else '❌ FAIL'}")
        
        print(f"\n📂 Category Breakdown:")
        for category, stats in analysis['category_breakdown'].items():
            print(f"{category:15} {stats['accuracy']:.1%} ({stats['correct']}/{stats['total']})")
        
        print(f"\n📈 Difficulty Breakdown:")
        for difficulty, stats in analysis['difficulty_breakdown'].items():
            print(f"{difficulty:15} {stats['accuracy']:.1%} ({stats['correct']}/{stats['total']})")
        
        # Overall pass/fail
        overall_pass = analysis['meets_accuracy_target'] and analysis['meets_latency_target']
