...
def main():
    ...
    while True:
        ...
        trigger_reason = (
            detector.get_trigger_reason(query)
            if should_trigger
        ...
        if should_trigger:
            ...
            for i, result in enumerate(results, 1):
                ...
                table.add_row(
                    str(i), f"{result['score']:.3f}", result["doc_id"], text_preview
                )
        ...
        if should_trigger and elapsed_time < 10:
            ...
            console.print(
                f"[bold red]✗ Retrieval target not met (target: <10 ms, actual: {elapsed_time:.2f} ms)[/bold red]"
            )
            console.print(
                "[dim]Note: This is expected with the simple retriever. FAISS-GPU will meet the target.[/dim]"
            )
