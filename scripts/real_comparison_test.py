...
def run_sanad_enhanced_test(query: str):
    ...
    try:
        ...
        print(f"Trigger Decision: {'YES' if trigger_result['should_trigger'] else 'NO'}")
        ...
        if results:
            ...
            confidence = "High" if sanad_score > 0.8 else "Medium" if sanad_score > 0.6 else "Low"
        else:
            sanad_score = 0.3
            confidence = "Low"
        ...
        print(f"- Average relevance: {avg_relevance:.3f}" if results else "- No relevant documents found")
...
def compare_results(baseline_result, baseline_time, sanad_result, sanad_time):
    ...
    if baseline_result and sanad_result:
        ...
        same_answer = baseline_result.get("answer", "") == sanad_result.get("answer", "")
    ...
    print("|------------------------|------------------|----------------------|")
    print(f"| Response Time          | {baseline_time}ms | {sanad_time}ms (+{((sanad_time-baseline_time)/baseline_time*100):.0f}%) |" if baseline_time > 0 else "| Response Time          | N/A              | N/A                  |")
    
    if baseline_result and sanad_result:
        source_count = len(sanad_result.get("sources", []))
        sanad_score = sanad_result.get("sanad_score", 0)
        print(f"| Source Verification    | ❌ None           | ✅ {source_count} documents       |")
        print(f"| Trust Scoring          | ❌ None           | ✅ {sanad_score:.2f}/1.00        |")
    ...
    if sanad_result and len(sanad_result.get("sources", [])) > 0:
        print("This query benefited from Sanad - real documents were found and verified!")
    else:
...
def main():
    ...
    try:
        choice = input(f"\nEnter query number (1-{len(test_queries)}) or press Enter for query 1: ").strip()
