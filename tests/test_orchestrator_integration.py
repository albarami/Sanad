...
class TestOrchestratorEnhancerIntegration:
    ...
    async def test_orchestrator_with_enhancement_needed(
        self,
        enhancer_llm_class,
        enhancer_config,
        orch_llm_class,
        ...
        coordinator.register_agent(
            MockAgent("PrecisionAgent", 0.5, "Answer lacks detail")
        )
        coordinator.register_agent(
            MockAgent("ProvenanceAgent", 0.4, "Missing citations")
        )
    ...
    @patch("core.enhancer.get_config")
    @patch("core.enhancer.BaselineLLM")
    async def test_orchestrator_without_enhancement(
        self,
        enhancer_llm_class,
        enhancer_config,
        orch_llm_class,
        ...
        assert (
            response.answer == coordinator_input.draft_answer
    ...
    @patch("core.enhancer.get_config")
    @patch("core.enhancer.BaselineLLM")
    async def test_orchestrator_enhancement_failure_fallback(
        self,
        enhancer_llm_class,
        enhancer_config,
        orch_llm_class,
        ...
        # Register agents with low scores to trigger enhancement
        coordinator.register_agent(
            MockAgent("PrecisionAgent", 0.5, "Needs improvement")
        )
    ...
    @patch("core.enhancer.get_config")
    @patch("core.enhancer.BaselineLLM")
    async def test_orchestrator_with_no_agents(
        self,
        enhancer_llm_class,
        enhancer_config,
        orch_llm_class,
        ...
        assert (
            response.answer != coordinator_input.draft_answer
        )  # Should be enhanced due to low score
        assert response.processing_time_ms > 0
    ...
    @patch("core.enhancer.get_config")
    @patch("core.enhancer.BaselineLLM")
    async def test_orchestrator_agent_timeout_handling(
        self,
        enhancer_llm_class,
        enhancer_config,
        orch_llm_class,
        ...
        class TimeoutAgent(BaseAgent):
            ...
            async def evaluate(self, input_data: AgentInput) -> AgentScore:
                ...
                return AgentScore(
                    score=0.8, explanation="", confidence=0.8, agent_name="timeout"
                )
